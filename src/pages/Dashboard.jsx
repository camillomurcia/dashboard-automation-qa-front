...
export default function Dashboard({ funcionalidades, onSelect, onCrear, isLoading = false }) {
        ...
        <div className="min-h-screen p-4 md:p-8 max-w-7xl mx-auto">
            <header className="mb-10 flex flex-col md:flex-row justify-between md:items-center gap-4">
                ...
                <button 
                    ...
                    disabled={isLoading}
                    className="bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-400 disabled:cursor-not-allowed text-white px-5 py-2.5 rounded-xl font-medium flex items-center justify-center gap-2 transition-all shadow-lg shadow-indigo-500/30"
                >
                ...
                {isLoading ? (
                    ...
                    funcionalidades.map(func => (
                        <div key={func.id} onClick={() => onSelect(func.id)} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:shadow-xl hover:border-indigo-300 transition-all cursor-pointer group">
                            ...
                            <h3 className="text-xl font-semibold mb-2 text-slate-800">{func.nombre || 'Cargando...'}</h3>
                            <p className="text-slate-500 text-sm line-clamp-2">{func.descripcion || 'Obteniendo descripción...'}</p>
                        </div>
                    ))
                )}
            </div>
